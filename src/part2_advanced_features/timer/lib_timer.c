...
// <<< end of configuration section >>>    
  ...
  else if ( timer_num == 1 )
  {
	...
	else if ( timer_num == 2 ) // TIMER 2: audio frequency
  {
    // Same as timer 0 of audio template
    LPC_SC->PCONP |= (1 << 22); 
    
    LPC_TIM2->MR0 = TimerInterval;
    LPC_TIM2->MCR = 3; // Interrupt and Reset over MR0 (continous wave)
    NVIC_EnableIRQ(TIMER2_IRQn);
    // Priorità alta per l'audio per evitare distorsioni
    // NVIC_SetPriority(TIMER2_IRQn, 0); 
    return (1);
  }
  else if ( timer_num == 3 ) // TIMER 3: note duration
  {
    // Same as timer 1 of audio template
    // PCONP bit 23 per TIM3
    LPC_SC->PCONP |= (1 << 23);

    LPC_TIM3->MR0 = TimerInterval;
    LPC_TIM3->MCR = 7; // Interrupt, Reset and STOP over MR0 (One-shot)
